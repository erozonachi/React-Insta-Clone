...
- State-driven views are views whose look, feel and structure highly depend on the ever changing app data, while in active interaction with it's users. They are important to us as software developers, because they aid us build complex and powerful applications that aim at a great user experience. They also make life easier for developers as they allow for code reusability where there are repeating patterns. React enables this paradigm through it's state and component lifecycle management modules/APIs (or useState and useEffect hooks).
